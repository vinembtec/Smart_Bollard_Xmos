...
#define EXPLORERCOMPILE 0

#define PRINT_FN(...) printf(__VA_ARGS__); printf("\n");

//#define this_tile(x) tile[1]
...
void Si_SetupLPDDRPadCtrl () {
//  unsigned node;
  
//  node = this_tile();

  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_CLK_ADDR,   BOARD_DDR_PADCTRL_CLK_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_CKE_ADDR,   BOARD_DDR_PADCTRL_CKE_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_CS_N_ADDR,  BOARD_DDR_PADCTRL_CS_N_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_WE_N_ADDR,  BOARD_DDR_PADCTRL_WE_N_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_CAS_N_ADDR, BOARD_DDR_PADCTRL_CAS_N_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_RAS_N_ADDR, BOARD_DDR_PADCTRL_RAS_N_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_ADDR_ADDR,  BOARD_DDR_PADCTRL_ADDR_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_BA_ADDR,    BOARD_DDR_PADCTRL_BA_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_DQ_ADDR,    BOARD_DDR_PADCTRL_DQ_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_DQS_ADDR,   BOARD_DDR_PADCTRL_DQS_VAL);
  write_sswitch_reg(get_tile_id(tile[1]), PADCTRL_CSR_BASE + PADCTRL_CSR_DM_ADDR,    BOARD_DDR_PADCTRL_DM_VAL);
...
//  unsigned node;
  ...
  if (!write_sswitch_reg(get_tile_id(tile[1]), LPDDR_CONTROLLER_CSR_BASE + LPDDR_CSR_ARBITRATION_MTG_COMMAND, wdata)) TerminateFail(0xdd7);

  // poll until we see pausing has occurred
  for (i=0; i<100; i++) {
    retval = read_sswitch_reg(get_tile_id(tile[1]), LPDDR_CONTROLLER_CSR_BASE + LPDDR_CSR_ARBITRATION_MTG_COMMAND, rdata);
    if (!retval) {
...
//  unsigned node;  
//  node = this_tile();
  if (!write_sswitch_reg(get_tile_id(tile[1]), LPDDR_CONTROLLER_CSR_BASE + LPDDR_CSR_ARBITRATION_MTG_COMMAND, 0)) TerminateFail(0xdd7);
  return 1;
}
...
//  PRINT_FN("DEBUG: LPDDRRecal: supplied write_dll_setting = %d, calculated read setting = %d", write_dll_setting, read_recal_val);
  ...
  if (!write_sswitch_reg(get_tile_id(tile[1]), LPDDR_CONTROLLER_CSR_BASE + LPDDR_CSR_DLL_PHY_CALIBRATION_DATA, wdata)) TerminateFail(0xdd8);

  // just read back reg to check write worked ok
  retval = read_sswitch_reg(get_tile_id(tile[1]), LPDDR_CONTROLLER_CSR_BASE + LPDDR_CSR_DLL_PHY_CALIBRATION_DATA, rdata);
  if (!retval) {
...
//  unsigned node;
  ...
  if (!write_sswitch_reg(get_tile_id(tile[1]), LPDDR_CONTROLLER_CSR_BASE + LPDDR_CSR_DLL_CONTROL, wdata)) TerminateFail(0xdd4);
  ...
  retval = read_sswitch_reg(get_tile_id(tile[1]), LPDDR_CONTROLLER_CSR_BASE + LPDDR_CSR_DLL_MEASUREMENT_STATUS, rdata);
  if (!retval) {
  ...
  if (!write_sswitch_reg(get_tile_id(tile[1]), LPDDR_CONTROLLER_CSR_BASE + LPDDR_CSR_DLL_CONTROL, wdata)) TerminateFail(0xdd5);
...
#if 1
  write_sswitch_reg(get_tile_id(tile[1]), ddrClkDivAddr, ddr_divider_value); // set the ddr clock divide ratio
#else  
...
write_sswitch_reg(get_tile_id(tile[1]), ddrClkDivAddr, ddr_divider_value); // set the ddr clock divide ratio
  ...
  write_sswitch_reg(get_tile_id(tile[1]), lpddrControllerConfigAddr, 0);   // "dummy" write, should not change reset values
//  write_sswitch_reg(get_tile_id(tile[1]), lpddrControllerConfigAddr, 1 | (1 << get_tile_id(tile[1])));   // enabled, accessed from either tile
  write_sswitch_reg(get_tile_id(tile[1]), lpddrControllerConfigAddr, 1);   // enabled, accessed from either tile
  ...
  if (!write_sswitch_reg(get_tile_id(tile[1]), LPDDR_CONTROLLER_CSR_BASE + LPDDR_CSR_EMR_OPCODE, wdata)) TerminateFail(0xdd2);
#endif
  ...
  retval = read_sswitch_reg(get_tile_id(tile[1]), regaddr, rdata);   // read it first and expect 1
  ...
  if (!write_sswitch_reg(get_tile_id(tile[1]), regaddr, wdata)) TerminateFail(0xdd3);
  ...
  if (!write_sswitch_reg(get_tile_id(tile[1]), regaddr, wdata)) TerminateFail(0xdd4);
  
  // read back, to check
  retval = read_sswitch_reg(get_tile_id(tile[1]), regaddr, rdata);
  ...
  if (!write_sswitch_reg(get_tile_id(tile[1]), regaddr, wdata)) TerminateFail(0xdd6);
...
//  PRINT_FN("DEBUG: Si_SetupLPDDR_SysPLL: tWR=%d, tXSR=%d, tRAS=%d, tREFI=%d", twr_value, txsr_value, tras_value, trefi_value);
  ...
  if (!write_sswitch_reg(get_tile_id(tile[1]), regaddr, wdata)) TerminateFail(0xdd7);
...
#define PSC_CTRL0_EXTMEM_ENABLE 0
psc_val = getps(XS1_PS_XCORE_CTRL0);
setps(XS1_PS_XCORE_CTRL0, psc_val | (1 << PSC_CTRL0_EXTMEM_ENABLE));



unsigned node = get_local_tile_id();

write_sswitch_reg(node, SSWITCH_SS_LPDDR_CONTROLLER_CONFIG_NUM, 1 | (1 << 1)); // enabled, accessed from tile 1
...
void setup_ddr() {
  Si_SetupLPDDRPadCtrl();
  Si_SetupLPDDR_SysPLL(100); 
  return;
}
