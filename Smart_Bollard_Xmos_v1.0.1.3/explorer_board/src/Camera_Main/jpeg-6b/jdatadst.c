...
//port_out(XS1_PORT_4C, 0x09);	  
	UINT bw;  
	FRESULT fr;
	fr = f_write(dest->outfile, dest->buffer, OUTPUT_BUF_SIZE, &bw);
	if (bw < 1) ERREXIT(cinfo, JERR_FILE_WRITE);
...
//port_out(XS1_PORT_4C, 0x09);		
	UINT bw;
	FRESULT fr;
	fr = f_write(dest->outfile, dest->buffer, datacount, &bw);
	if (bw < datacount) ERREXIT(cinfo, JERR_FILE_WRITE);
resetGpio(PORT_LEDS_X1D4C0);
//port_out(XS1_PORT_4C, 0x00);
  }
  //fflush(dest->outfile);
...
GLOBAL(void)
jpeg_stdio_dest (j_compress_ptr cinfo, FIL * outfile)
{
  my_dest_ptr dest;
